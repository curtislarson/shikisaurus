import { IThemedToken } from "../deps.ts";
import { FontManager, FontNames, FontStyle } from "./font-manager.ts";
...
export interface SVGRendererOptions {
   ...
   * @example
   *
   * getSVGRenderer({ fontFamily: 'Fira_Code' })
   */
  fontFamily: FontNames; // | RemoteFontFamily;
  /**
  ...
  fontSize?: number;
  /**
   * How high should a line be, in relation to `fontSize`. Default to 1.5.
...
export class SvgRenderer {
  ...
  constructor(opts: { fonts: FontManager }) {
    this.fonts = opts.fonts;
  }

  async getRenderer(options: SVGRendererOptions) {
    const fontNameStr = options.fontFamily;
    const fontSize = options.fontSize ?? 16;
    const lineHeightToFontSizeRatio = options.lineHeightToFontSizeRatio ?? 1.5;
    const _bg = options.bg ?? "#000";
    ...
    const measurement = await this.fonts.measureFont(fontNameStr, fontSize);
    ...
    return {
      renderToSVG(lines: IThemedToken[][], { bg } = { bg: _bg }) {
         ...
         */
        const bgWidth = (longestLineTextLength + bgSideCharPadding * 2)
          * measurement.width;
        /**
         * all rows + 2 rows top/bot
         */
        const bgHeight = (lines.length + bgVerticalCharPadding * 2)
          * lineheight;
          ...
          if (l.length === 0) {
            ...
            l.forEach((token) => {
              ...
              if (
                ...
                svg += `<tspan x="${indent * measurement.width}" ${tokenAttributes}>${
                  escapeHtml(
                    token.content.slice(0, firstNonWhitespaceIndex),
                  )
                }</tspan>`;

                svg += `<tspan x="${(indent + firstNonWhitespaceIndex) * measurement.width}" ${tokenAttributes}>${
                  escapeHtml(token.content.slice(firstNonWhitespaceIndex))
                }</tspan>`;
              } else {
                svg += `<tspan x="${indent * measurement.width}" ${tokenAttributes}>${
...
function getTokenSVGAttributes(token: IThemedToken) {
  const options: Record<string, string | undefined | number> = {
    fill: "#fff",
    opacity: undefined,
  ...
  if (token.color != null && token.color.slice(1).length <= 6) {
    options.fill = token.color;
  } else if (token.color != null && token.color.slice(1).length === 8) {
